...
class FedXGBCTrainer:
    def __init__(self, traindata: pd.DataFrame, get_importance: bool = False):
    ...
    def _grad(self, preds, labels):
        preds = self.sigmoid(preds)
        return(preds - labels)
    
    # second order gradient logLoss
    def _hess(self, preds):
        preds = self.sigmoid(preds)
        return(preds * (1 - preds))
    
    def init_histogram(self, feature_splits):
